...
def _build_global_deploy_overrides(optimism_args):
    ...
    # QKC changes
    if optimism_args.op_contract_deployer_params.overrides.get("deploySoulGasToken", False):
        if not global_overrides:
            global_overrides = {}
        global_overrides["deploySoulGasToken"] = True
        global_overrides[
    ...
    # QKC changes
    if optimism_args.op_contract_deployer_params.overrides.get("l2GenesisBlobTimeOffset", False):
        if not global_overrides:
            global_overrides = {}
        global_overrides[
